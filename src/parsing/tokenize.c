...
Parser *tokenize(char *str) {
	...
	while (*str) {
		...
		if (!single_quotes) {
			PARSE_TOKEN("${", "}", t_var);
			PARSE_TOKEN_NESTED("$((", "))", t_arithmetic);
			PARSE_TOKEN_NESTED("$(", ")", t_control_substitution);
			PARSE_VARIABLE(t_wordvar);
		}
		if (!single_quotes && !double_quotes) {
			PARSE_TOKEN_NESTED("(", ")", t_subshell);
			PARSE_TOKEN_NESTED("{", "}", t_control_group);
			PARSE_TOKEN_NESTED(">(", ")", t_process_substitution_to);
			PARSE_TOKEN_NESTED("<(", ")", t_process_substitution_from);
